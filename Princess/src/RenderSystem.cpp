...
void RenderSystem::Update()
{
	for (int i = 0; i < m_entities.size(); i++)
	{
		...
		if (pcKey >= 0 && scKey >= 0)
		{
			...
			SDL_Rect* holder = new SDL_Rect{ (int)static_cast<PositionComponent*>(m_entities.at(i)->GetComponents()->at(pcKey))->getX(), (int)static_cast<PositionComponent*>(m_entities.at(i)->GetComponents()->at(pcKey))->getY(), static_cast<SpriteComponent*>(m_entities.at(i)->GetComponents()->at(scKey))->Width() * m_scale, static_cast<SpriteComponent*>(m_entities.at(i)->GetComponents()->at(scKey))->Height()*m_scale };
