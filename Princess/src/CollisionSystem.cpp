...
void CollisionSystem::Update()
{
	for (int i = 0; i < m_entities.size(); i++)
	{
		...
		for (int j = 0; j < m_entities.at(i)->GetComponents()->size(); j++)
		{
			...
			else if (m_entities.at(i)->GetComponents()->at(j)->Type() == "PC")
			{
				pcKey = j;
			}
		}
		...
		if (ccKey >= 0)
		{
			if (m_entities.at(i)->ID() == "Player")
			{
				...
				for (int j = 0; j < m_entities.size(); j++)
				{
					...
					{
						if (m_entities.at(j)->GetComponents()->at(k)->Type() == "collision")
						{
							ccOtherKey = k;
						}
						else if (m_entities.at(j)->GetComponents()->at(k)->Type() == "PC")
						{
							pcOtherKey = k;
						}
					}
					...
					if (ccOtherKey >= 0)
					{
						if (m_entities.at(j)->ID() == "Projectile")
						{
							int x1 = static_cast<PositionComponent*>(m_entities.at(i)->GetComponents()->at(pcKey))->getPosition().x;
							int y1 = static_cast<PositionComponent*>(m_entities.at(i)->GetComponents()->at(pcKey))->getPosition().y;

							int x2 = static_cast<PositionComponent*>(m_entities.at(j)->GetComponents()->at(pcOtherKey))->getPosition().x;
							int y2 = static_cast<PositionComponent*>(m_entities.at(j)->GetComponents()->at(pcOtherKey))->getPosition().y;


							int w1 = static_cast<SpriteComponent*>(m_entities.at(i)->GetComponents()->at(ccKey))->GetRect().w;
							int h1 = static_cast<SpriteComponent*>(m_entities.at(i)->GetComponents()->at(ccKey))->GetRect().h;

							int w2 = static_cast<SpriteComponent*>(m_entities.at(j)->GetComponents()->at(ccOtherKey))->GetRect().w;
							int h2 = static_cast<SpriteComponent*>(m_entities.at(j)->GetComponents()->at(ccOtherKey))->GetRect().h;
