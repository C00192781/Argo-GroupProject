...
void MovementSystem::Update()
{
	...
	for (int i = 0; i < m_entities.size(); i++)
	{
		...
		{
			position = std::static_pointer_cast<RectangleComponent>(m_entities.at(i)->GetComponents()->at(rectangleKey))->getPositionPtr();
			velocity = std::static_pointer_cast<MovementComponent>(m_entities.at(i)->GetComponents()->at(mcKey))->getVelocityPtr();
